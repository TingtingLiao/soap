...
# from lib.geo_.remesh import calc_edge_length, calc_edges, calc_face_collapses, calc_face_normals, calc_vertex_normals, collapse_edges, flip_edges, pack, prepend_dummies, remove_dummies, split_edges
...
def remesh(
    ...
    vertices_etc, _ = collapse_edges(vertices_etc,faces,edges,priority)
    ...
    if vertices.shape[0]<max_vertices:
        ...
        if vid_mask is not None: 
            vid_mask = torch.cat((torch.tensor([True], device=vid_mask.device), vid_mask))
            splits = splits & vid_mask[edges].any(dim=-1)
...
def remesh_withuv(
    ...
    # print('before split:', vertices_etc.shape, faces.shape, vt.shape, ft.shape)
    ...
    if vertices.shape[0]<max_vertices:
        ...
        if vt is not None: 
            vertices_etc, faces, vt, ft = split_edges(
        ...
        # if vt is not None: 
        #     vt,ft = split_edges(vt,ft,ft_edges,ft_to_edge,splits,pack_faces=False)

    vertices_etc,faces, used_vertices = pack(vertices_etc,faces, True)
    vertices = vertices_etc[:,:3]
    ...
    new_v, new_f = remove_dummies(vertices_etc,faces)

    if vt is not None:
        new_vt, new_ft = remove_dummies(vt,ft)
        return new_v, new_f, new_vt, new_f
...
def remesh_withuv_(
    ...
    if vertices.shape[0]<max_vertices:
        if vt is not None: 
            edges,face_to_edge, ft_edges, ft_to_edge = calc_edges(faces, ft=ft) #E,2 F,3
        else:
            edges,face_to_edge = calc_edges(faces) #E,2 F,3
        ...
        if vt is not None: 
            vertices_etc, faces, vt, ft = split_edges(
                vertices_etc, faces, edges, face_to_edge, splits, pack_faces=False,
                vt=vt, ft=ft, ft_edges=ft_edges,
                ft_to_edge=ft_to_edge
                )
        else:
            vertices_etc,faces = split_edges(vertices_etc,faces,edges,face_to_edge,splits,pack_faces=False)
    ...
    print('after pack', vertices_etc.shape, faces.shape, vt.shape, ft.shape)
    ...
    if vt is not None:
        new_vt, new_ft = remove_dummies(vt,ft)
        return new_v, new_f, new_vt, new_ft
