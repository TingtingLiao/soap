...
def calc_edges(
    ...
    full_edges = face_edges.reshape(F*3,2)  #F*3,2
    sorted_edges, sort_ids = full_edges.sort(dim=-1) #F*3,2 TODO min/max faster?
    
    # make unique edges
    edges, unique_to_full, count = torch.unique(input=sorted_edges, sorted=True, return_inverse=True, return_counts=True, dim=0) #(E,2),(F*3)
    E = edges.shape[0]
    face_to_edge = unique_to_full.reshape(F,3) #F,3
    
    if ft is not None:
        ft_edges = torch.stack((ft, ft.roll(-1, 1)), dim=-1)  # F*3,3,2
        full_ft_edges = ft_edges.reshape(F*3, 2)
        
        # mapping for sort 
        sorted_ft_edges = torch.gather(full_ft_edges, dim=-1, index=sort_ids)

        # mapping for unique  
        ids = torch.stack([
            torch.nonzero(unique_to_full==i, as_tuple=True)[0][0] for i in range(E)
        ])
        tmp = sorted_edges[ids]  
        ft_edges = sorted_ft_edges[ids]  # F,3
    
        return edges, face_to_edge, ft_edges, face_to_edge
    ...
    edge_to_face.reshape(2*E,2).scatter_(dim=0,index=(2*unique_to_full+is_right)[:,None].expand(F*3,2),src=scatter_src) #E,LR=2,S=2
    edge_to_face[0] = 0
...
def pack(
    ...
    if return_indices:
        return vertices,faces,used_vertices
...
def split_edges(
    ...
    if vt is not None and ft is not None:  
        # edge_vt = torch.zeros_like(splits, dtype=torch.long)  # E
        # edge_vt[splits] = torch.arange(len(vt), len(vt) + S, dtype=torch.long, device=vertices.device)  # E 0 for no split, sync
        # side_vt = edge_vt[ft_to_edge]
        # split_vt_edges = ft_edges[splits]  
        
        # # vt 
        # split_vt = vt[split_vt_edges].mean(dim=1)  
        # vt = torch.concat((vt, split_vt), dim=0) 
        
        # # ft  
        # side_split = side_vt != 0  # F,3
        # shrunk_ft = torch.where(side_split, side_vt, ft) #F,3  
        # new_ft = side_split[:,:,None] * torch.stack((ft, side_vt, shrunk_ft.roll(1, dims=-1)), dim=-1) #F,3,3
        # ft = torch.concat((shrunk_ft, new_ft.reshape(F*3, 3))) #4F,3
        
        new_vt = vt[split_edges].mean(dim=1) 
        vt = torch.concat((vt, new_vt), dim=0) 
        
        
        return vertices, faces, vt, faces
