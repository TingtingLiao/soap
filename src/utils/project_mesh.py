...
def multiview_color_projection(meshes: Meshes, 
                               image_list: List[Image.Image], 
                               cameras_list: List[CamerasBase]=None, 
                               camera_focal: float = 2 / 1.35, 
                               weights=None, eps=0.05, resolution=1024, 
                               device="cuda", 
                               reweight_with_cosangle="square", 
                               use_alpha=True, 
                               confidence_threshold=0.1, 
                               complete_unseen=False, 
                               below_confidence_strategy="smooth") -> Meshes:
    """
    Projects color from a given image onto a 3D mesh.
    ...
    if len(image_list) == 4:
        ...
        cameras_list = get_cameras_list([0, 90, 180, 270, 45, 315], "cuda", focal=1)
    
    if weights is None:  
        if len(image_list) == 4:
            weights = [2.0, 0.2, 1.0, 0.2] 
        else:
            weights = [2.0, 0.2, 1.0, 0.2, 0.05, 0.05]
    
    # 2. run projection
    meshes = meshes.clone().to(device) 
    # assert len(cameras_list) == len(image_list) == len(weights) 
    print(f"len(cameras_list)={len(cameras_list)}, len(image_list)={len(image_list)}, len(weights)={len(weights)}")
...
def get_6view_cameras(device, focal=2/1.35):
    ...
    return get_cameras_list(azim_list = [180, 270, 0, 90, 225, 135], device=device, focal=focal)
...
def get_multiple_view_cameras(device, focal=2/1.35, offset=180, num_views=8, dist=1.1):
    return get_cameras_list(azim_list = 
                            (np.linspace(0, 360, num_views+1)[:-1] + offset) % 360, 
                            device=device, focal=focal, dist=dist)
